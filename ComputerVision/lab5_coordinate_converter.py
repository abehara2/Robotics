...
def IMG2W(r,c):
    ...
    Ir = (r - Or)/beta
    Ic = (c - Oc)/beta
    translate = np.asarray([Ir - tx, Ic - ty, 0])
    
    RotZ = np.zeros((3,3))
    RotZ[0] = [np.cos(theta), -np.sin(theta), 0]
    RotZ[1] = [np.sin(theta), np.cos(theta), 0]
    RotZ[2] = [0,0,1]

    WCoords = np.matmul(translate, RotZ)
    #WCoords = np.transpose(WCoords)
